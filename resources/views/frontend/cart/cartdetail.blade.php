						...
						<div class="wrap-table-shopping-cart">
							<table class="table-shopping-cart">
								<tr class="table_head" style="width:400px">
									...
									<th class="column-7" style="width: 10%;">Total</th>
								</tr>
                                ...
                                <tr class="table_row">
									...
									<td class="column-4">
										<div class="wrap-num-product flex-w m-l-auto m-r-0">
											...
											<input class="mtext-104 cl3 txt-center num-product" type="number" name="num-product" value="{{$item->qty}}">
                                    ...
                                    <td class="column-5">{{$item->options->color}}</td>
                                    <td class="column-6">{{$item->options->size}}</td>
									<td class="column-7">{{'$'.$item->subtotal}}</td>
								</tr>
                                @endforeach
    ...
    <script>
        $(function(){
            ...
            $('.btn-num-product-down').click(function(){
                var inputElement = $(this).closest('.wrap-num-product').find('.num-product');
                var num=inputElement.val();
                var idPd=$(this).data('id');
                var subtotal = $(this).closest('tr').find('.column-7');
                if(num==0){
                ...
                $.ajax({
                        ...
                        success:function(result){
						   ...
						   var cartItemSelector = '.header-cart-item-info[data-product-id="'+result.data_id+ '"]';
